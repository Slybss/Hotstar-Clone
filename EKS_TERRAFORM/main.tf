...
# Ensure at least two subnets in different availability zones
locals {
  fallback_subnet_ids = ["subnet-0bb1c79de3EXAMPLE1", "subnet-0bb1c79de3EXAMPLE2"] # Replace with actual subnet IDs
  effective_public_subnet_ids = length(local.public_subnet_ids) >= 2 ? local.public_subnet_ids : local.fallback_subnet_ids
}
